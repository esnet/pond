...
export class Event {
     ...
     */
    constructor(arg1: Date | Moment | number, arg2: any | Map<any, any> | number);
     ...
     */
    static combine(events: Event[], fieldSpec: string | string[], reducer: Function): void;
     ...
     *      fieldSpec string | array - Column or columns to look up. If you need to retrieve multiple deep nested values that ['can.be', 'done.with', 'this.notation']. A single deep value with a string.like.this. If not supplied, all columns will be operated on.
     */
    static avg(events: Event[], fieldSpec: string | string[]): void;
...
export class TimeRangeEvent {
     ...
     */
    collapse(fieldSpecList: string | string[], name: any, reducer: Function): any;
}
...
export class Collection {
     ...
     */
    constructor(arg1: Collection | any[], arg2: boolean);
     ...
     */
    eventListAsArray(): Array<Event> | Array<TimeRangeEvent> | Array<IndexedEvent>
     ...
     */
    filter(func: Function): Collection;
     ...
     *      filter function - Optional filter function used to clean data before aggregating
     */
    percentile(q: number, fieldPath: string | string[], interp: "linear" | "lower" | "higher" | "nearest" | "midpoint", filter: Function): number;
     ...
     *      interp string = "linear" - Specifies the interpolation method to use when the desired quantile lies between two data points. Options are: options are: * linear: i + (j - i) * fraction, where fraction is the fractional part of the index surrounded by i and j. * lower: i. * higher: j. * nearest: i or j whichever is nearest. * midpoint: (i + j) / 2.
     */
    quantile(n: number, column: string, interp: "linear" | "lower" | "higher" | "nearest" | "midpoint"): Array<any>;
...
export class TimeSeries {
     ...
     * }
     */
    // TODO: re-enable property on typescript@2.3.x release
    events(): any;//Enumerator<Event> | Enumerator<TimeRangeEvent> | Enumerator<IndexedEvent>;
     ...
     */
    setCollection(collection: Collection, isChronological: boolean): TimeSeries;
     ...
     *      filter function - Optional filter function used to clean data before aggregating
     */
    percentile(q: number, fieldPath: string | string[], interpinterp: "linear" | "lower" | "higher" | "nearest" | "midpoint", filter: Function): number;
     ...
     */
    quantile(n: number, fieldPath: string | string[], interpinterp: "linear" | "lower" | "higher" | "nearest" | "midpoint"): any[];
     ...
     *
     */
    collapse(options: { fieldSpecList: string[], name: string, reducer: Function, append?: boolean }): TimeSeries;
     ...
     * });
     */
    renameColumns(options: { renameMap: { in: string, out: string } }): TimeSeries;
     ...
     *
     */
    fill(options: { fieldSpec: string | string[], method: "linear" | "pad" | "zero", limit: number }): TimeSeries;
     ...
     * });
     */
    align(options: { fieldSpec: string | string[], period: string, method: "linear" | "pad", limit: number }): TimeSeries;
     ...
     *          resets.
     */
    rate(options: { fieldSpec: string | string[], allowNegative: boolean }): TimeSeries;
     ...
     *     });
     */
    fixedWindowRollup(options: { windowSize: string, aggregation: any, toEvents: boolean }): TimeSeries;
     ...
     *          .aggregation object - The aggregation specification (see description above)
     */
    hourlyRollup(options: { aggregation: any, toEvents: boolean }): TimeSeries;
     ...
     *          .aggregation object - The aggregation specification (see description above)
     */
    dailyRollup(options: { aggregation: any, toEvents: boolean }): TimeSeries;
     ...
     *          .aggregation object - The aggregation specification (see description above)
     */
    monthlyRollup(options: { aggregation: any, toEvents: boolean }): TimeSeries;
     ...
     *          .aggregation object - The aggregation specification (see description above)
     */
    yearlyRollup(options: { aggregation: any, toEvents: boolean }): TimeSeries;
     ...
     */
    collectByFixedWindow(options: { windowSize: string }): Map<Index, Collection>;
     ...
     * string.like.this.
     */
    timeseriesListReduce(options: { seriesList: TimeSeries[], reducer: Function, fieldSpec: string | string[] }): TimeSeries;
     ...
     * });
     */
    timeSeriesListMerge(options: { seriesList: TimeSeries[], fieldSpec: string | string[] }): TimeSeries;
     ...
     * });
     */
    timeSeriesListSum(options: { seriesList: TimeSeries[], fieldSpec: string | string[] }): TimeSeries;
}
...
export class Pipeline {
     ...
     * const p = Pipeline()...
     */
    constructor(arg: Pipeline | Map<any, any> | undefined);
     ...
     */
    windowBy(w: string | any): Pipeline;
     ...
     */
    emitOn(trigger: "eachEvent" | "discard" | "flush"): Pipeline;
     ...
     */
    count(observer: Function, force: boolean): Pipeline;
     ...
     */
    offsetBy(by: number, fieldSpec: string | string[]): Pipeline;
     ...
     */
    asEvents(options: { [key: string]: any, alignment: "lag" | "center" | "lead" }): Pipeline;
     ...
     */
    select(fieldSpec: string | string[]): Pipeline;
     ...
     *      }, true);
     */
    collapse(fieldSpecList: string | string[], name: string, reducer: Function, append: boolean): Pipeline;
     ...
     */
    fill(options: { fieldSpec: string | string[], method: "zero" | "linear" | "pad" }): Pipeline;
     ...
     */
    asTimeRangeEvents(options: { duration: string, alignment: "front" | "center" | "behind" }): Pipeline;
     ...
     */
    asIndexedEvents(options: { duration: string }): Pipeline;
}