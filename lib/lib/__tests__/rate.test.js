...
var RATE = {
  name: "traffic",
  columns: ["time", "in"],
  points: [[0, 1], [30000, 3], [60000, 10], [90000, 40], [120000, 70], [150000, 130], [180000, 190], [210000, 220], [240000, 300], [270000, 390], [300000, 510]]
};
it("can calculate the rate using TimeSeries.rate()", () => {
  var ts = new _timeseries.default(RATE);
  var rate = ts.rate({
    fieldSpec: "in"
...
it("can replicate basic esmond alignment and rate", done => {
  ...
  var ts = new _timeseries.default(RAW_RATES);
  var rates = ts.align({
    fieldSpec: "value",
    period: "30s"
  }).rate();
...
it("can output nulls for negative values", done => {
  ...
  var rates1 = ts.align({
    fieldSpec: "value",
    period: "30s"
  }).rate(); //lower counter will produce negative derivatives
