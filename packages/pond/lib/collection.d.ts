...
import { AlignmentOptions, CollapseOptions, RateOptions } from "./types";
 ...
 */
export declare class Collection<T extends Key> extends Base<T> {
    /**
     * Rebuild the keyMap from scratch
     */
    protected static buildKeyMap<T extends Key>(events: Immutable.List<Event<T>>): Immutable.Map<string, Immutable.Set<number>>;
     ...
     * ```
     */
    map<U extends Key>(mapper: (event?: Event<T>, index?: number) => Event<U>): Collection<U>;
    /**
    ...
    protected clone(events: any, keyMap: any): Base<T>;
